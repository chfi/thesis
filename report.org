...
not until a way to "repair" legacy code is found, but a way to write
code that does not /become/ legacy code is
...
PureScript that both embeds an existing JavaScript application, as
well as provides novel functionality. The process, and resulting code,
were evaluated
...
Biodalliance (BD) is one such genome browser, which is written in JavaScript (JS)
and can be embedded in web pages\cite{down2011dalliance}. However, BD
is several years old, and has accumulated a lot of code and features,
which have led to difficulties when adding new functionality that is
desired in GN2.

The Graph Genetics Browser (GGB) was developed to solve these
problems, by embedding BD and wrapping the desired features of BD in a
new interface, as well as providing the groundwork for a new genome
browser independent of BD. The development of GGB served to evaluate
the hypothesis, which is described in the next section.

** Purpose

# TODO reword these
The purpose of the thesis was to evaluate purely functional
programming as a tool to:

1. work with and extend an existing legacy system
2. develop a "legacy-resistent" application

The legacy system in question was BD, the new application GGB. GGB
embeds the BD browser, so that the functionality of BD is gained while
...
network browser is also embedded.
...
i.e. on what actually makes a good language\cite[p.~252]{seibel2009coders}. That is
