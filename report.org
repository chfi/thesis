...
flame."  -- Robert C. Martin, Foreword to Working Effectively with Legacy Code \cite{feathers2004working}.
#+END_QUOTE
...
This definition does not capture the spirit of the concept, however. \citeauthor{Bennett1995}
gives another definition of legacy systems, which gets us closer:
"large software systems that we don't know how to cope with but that are vital to
...
is to leave it as is --- until it is decided, for whatever reason, that these changes are
