...
Biodalliance (BD) is a HTML5-based genome browser written in JavaScript
(JS).
# TODO REF
It is fast[ref], and can be configured to display various types of plots fairly easily,
# TODO REF
and supports a number of different data formats commonly used in bioinformatics.
# TODO REF
However, it has been in development for a long time
# TODO REF
and has shown itself to be difficult to extend.
# TODO REF/footnote, GSOC
...
# Outro: say that we decided to develop a new codebase that connects to BD but is independent -- GGB

*** Genetics Graph Browser

The Genetics Graph Browser (GGB) is the main product of this project.
It was developed as both an extension to BD, as well as a stand-alone
web application.

GGB was written in PS, and various FP techniques and idioms were used
in its development, some unique to PS, most applicable to many pure
FP languages.
