...
# TODO reword these
The purpose of the thesis is to evaluate purely functional
programming as a tool to:

1. work with and extend an existing legacy system
2. develop a "legacy-resistent" application

To do this, a legacy system is required, namely Biodalliance (BD), a
genome browser written in JavaScript (JS). GeneNetwork 2 (GN2) is a
web-based database and toolset for doing genetics online. One feature
yet to be implemented in GN2 is an interactive genome browser, like
BD. However, BD lacks some desired functionality, and previous
attempts to embed it have been hindered due to its legacy nature.

Instead, a new browser --- the Graph Genetics Browser (GGB) --- is
developed. GGB embeds the BD browser, so that the functionality of BD
is gained while providing a framework for a new browser without the
legacy baggage of BD. As initial new functionality, the Cytoscape.js (Cy.js)
graph network browser is also embedded.
