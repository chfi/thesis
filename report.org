...
# TODO do we need to talk about code smells here? I'm not sure.
#+begin_comment
We now have a simple definition of good and bad code. It remains to
be able to easily see if some piece of code is good or bad.
# TODO maybe remove [easily?]
This is not an easy question to answer, nor is there any reason to
think there is an objective answer. What we need is a set of heuristics
that can be applied, to give a rough idea of the code quality; some
examples of what kind of code makes it difficult to see what it does
and does not do.

These heuristics exists, and they are called "code antipatterns," or "code smells."

**** Code smells


# TODO Ref to beck & fowler, working with legacy code
...
# been the primary programming paradigm for decades,
# TODO REF
...
Discuss results -- each of the code parts (subsections from Results)
  Does the code give "answer" to the code smells?
  Is the GGB code "good" code wrt. our code smell definition?
  Is the GGB code "good" code more generally? (maybe skip this one if it's too much)
