...
** Tracks
   ...
   in the `tracks` property of the configuration object.
...
So, the browser takes a hands-off approach to BD tracks, and the only validation
...
from Javascript are type-safe. BD and Cy.js TrackSinks, respectively, should
...
*** TODO TrackManager
(code not written yet -- does it even fit here since there's no actual configuration of it?)

The TrackManager combines the provided TrackSources and Sinks, feeding
the actual events produced by the tracks through the pipes, as well as
takes care of "type-checking" the events, so that one event name cannot
refer to multiple structures.
